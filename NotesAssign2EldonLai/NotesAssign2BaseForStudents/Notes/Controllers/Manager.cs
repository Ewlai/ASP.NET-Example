...
namespace Notes.Controllers
{
    public class Manager
    {
        ...
        public EmployeeBaseWithAssociatedData GetEmployeeByIdWithAssociatedData(int id)
        {
            ...
            var fetchedObject = ds.Employees.Include("DirectReports").Include("Notes").SingleOrDefault(un => un.Id == id);
        ...
        public NoteBase GetNoteForAuthenticatedEmployee(int id)
        {
            ...
            var fetchedObject = ds.Notes.Include("Employee").SingleOrDefault(un => un.Id == id);
            
            // Test whether the user is the same as the security context user
            if (fetchedObject.Employee.IdentityUserId == User.Identity.Name)
            {
                // If yes, can return the object
